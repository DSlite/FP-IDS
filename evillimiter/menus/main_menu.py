...
from evillimiter.networking.utils import ValueConverter
...
class MainMenu(CommandMenu):
    def __init__(self, version, interface, gateway_ip, gateway_mac, netmask):
        ...
        remove_parser = self.parser.add_subparser('remove', self._remove_handler)
    ...
    def _auto_handler(self, args):
        ...
        def display(stdscr, interval):
            ...
            header_off = [
                ('ID', 5), ('IP address', 18), ('Hostname', hname_max_len + 2), ('Status', 8), ('Current (per s)', 20), ('Total', 16), ('Packets', 0)
            ]
            ...
            while True:
                ...
                for host, result in host_results:
                    if not host.limited and int(ValueConverter.byte_to_bit(result.download_total_size.value)) > max_bit.rate:
                        IO.discord("""```
IP: {}
MAC: {}
NAME: {}
Dilimit menjadi {} karena melebihi batas maksimum {}```
                        """.format(host.ip, host.mac, host.name, rate, max_bit))
        ...
        try:
            IO.discord("Melakukan monitoring pada sistem...\nLimit host menjadi **{}** jika melebihi batas maksimum **{}**".format(args.rate, args.maximum))
